...
//consider applying another condition to stop user if errors are not yet corrected
    ...
    {
        /* initialization of variables and controller window */
        int state = HOME, part_counter = 0, nozzle_errors_to_check, preplace_errors_to_check;
...
//                pi[component_list[i]].component_designation, pi[component_list[i]].component_footprint, pi[component_list[i]].component_value,
//                pi[component_list[i]].x_target, pi[component_list[i]].y_target, pi[component_list[i]].theta_target, pi[component_list[i]].feeder);
//
//        }
            ...
            switch (state)
            {
                ...
                case HOME:
                    ...
                    if(isSimulatorReadyForNextInstruction())
                    {
                        ...
                        {
                            setTargetPos(TAPE_FEEDER_X[pi[0].feeder]+20, TAPE_FEEDER_Y[pi[0].feeder]); //go to the first feeder in the list, +20 for the left nozzle
                            state = MOVE_TO_FEEDER;
...
//                            requested_theta_left = pi[part_counter-2].theta_target - errortheta;  //calculate misalignment of the part on the nozzle
//                            printf("Time: %7.2f             %19s  Left part misalignment error: %3.2f  Correction required: %3.2f\n", getSimulationTime()," ", errortheta, requested_theta_left);
...
//                        {
//                            double errortheta = getPickErrorTheta(CENTRE_NOZZLE);  //acquire the part misalignment from the look-up photo
//                            requested_theta_centre = pi[part_counter-1].theta_target - errortheta;  //calculate misalignment of the part on the nozzle
//                            printf("Time: %7.2f             %19s  Centre part misalignment error: %3.2f  Correction required: %3.2f\n", getSimulationTime()," ", errortheta, requested_theta_centre);
//                            state = FIX_NOZZLE_ERROR;
...
//                            requested_theta_right = pi[part_counter].theta_target - errortheta;  //calculate misalignment of the part on the nozzle
//                            printf("Time: %7.2f             %19s  Right part misalignment error: %3.2f  Correction required: %3.2f\n", getSimulationTime()," ", errortheta, requested_theta_right);
//                            state = FIX_NOZZLE_ERROR;
...
//                        }
//
//                    if (isSimulatorReadyForNextInstruction() && lookdown_photo == TRUE)
//                    {
//                        preplace_diff_x = pi[part_counter].x_target - (pi[part_counter].x_target+getPreplaceErrorX()); //calculate the difference between the required x position and the actual x position of the gantry
//                        preplace_diff_y = pi[part_counter].y_target - (pi[part_counter].y_target+getPreplaceErrorY()); //calculate the difference between the required y position and the actual y position of the gantry
